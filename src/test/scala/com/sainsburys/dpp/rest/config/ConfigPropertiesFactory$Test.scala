...
class ConfigPropertiesFactory$Test extends FunSuite {
  ...
  test("testLoadConfigFromFile") {
    ...
    assert(configProperties.apiName.equals("test"))
    assert(configProperties.apiUrl.equals("https://test"))
    assert(configProperties.apiApiClientId.equals( "test"))
  ...
  test("testLoadConfigFromArguments") {
    ...
    args(1) = "api.webservice.url=https://test-new"
    ...
    assert(configProperties.apiUrl.equals("https://test-new"))
